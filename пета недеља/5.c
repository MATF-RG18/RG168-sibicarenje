...
//nivo koji se igra
int nivo = 0;
//ubrzanje ce se mnoziti sa nivoom kako bi se dobila veca brzina
double ubrzanje = 0.01;
//checkpoint
int checkpoint = 0;
//promenljiva pogodak koja oznacava da li je dobro odabrana casa
int pogodak = 0;


//funkcija ispis ispisuje tekst koji se prosledjuje kao argument funkcija
//pocevsi od (x,y,z) i u vecoj ili manjoj velicini zavisno od vrednosti
//promenljive velicina
void ispis(GLfloat x, GLfloat y, GLfloat z, char* text, int velicina)
{
    glPushMatrix();
    glTranslatef(x, y, z);
    glRotatef(90, 1, 0, 0);
    glRotatef(90, 0, 1, 0);
    glRotatef(270, 1, 0, 0);
    //velicina oznacava da li zelimo tekst normalne velicine ili dodatno umanjene
    //velicina = 0 je normalna velicina
    if(velicina == 0)
        glScalef(1/200.0, 1/250.0, 1/200.0);
    else
        glScalef(1/300.0, 1/350.0, 1/200.0);
    
    //postavljamo materijal za tekst
    GLfloat ambijent_teksta[] = { 2.05, 3.5, 2.25, 1 };
    glMaterialfv(GL_FRONT, GL_AMBIENT, ambijent_teksta);
    
    //pronadjena funkcija na internetu, zatim u man stranama istrazena
    //sluzi za ispisivanje teksta koji moze da se rotira, skalira itd
    for( char* p = text; *p; p++)
...
static void on_timer3(int value){
    ...
    if (animacija) {
        ...
        if(kuglica != odabrana_casa){
            ...
            //u suprotnom stavljamo da je pogodak = 1 jer zelimo da ispisemo poruku
            pogodak = 1;
    }
...
static void on_timer2(int value)
{
    ...
    if (animacija == 0){
        ...
        //ubacujemo ubrazanje i nivo
        //sto je veci (tezi) nivo, ubrzanje je vece
        //te je teze pogoditi u kojoj se casi nalazi kuglica
        ugao0 += (0.05+ubrzanje*nivo);
        glutPostRedisplay();
        if(ugao0 > PI/2)
            animacija = 0;        
...
static void on_display(void){
    ...
    sto();
     ...
     if(pogodak == 1){
        ispis(3,-1.5,0.85, "POGODAK", 0);
       
        //zelimo checkpoint na svakom petom nivou 0, 5, 10, 15
        if(nivo < 20 && nivo % 5 == 0){
            //saljemo odgovarajucu poruku o tome da se doslo do checkpoint-a
            char checkpoint[50] = "Checkpoint: Nivo = ", nivo_niska[10];
            sprintf(nivo_niska, "%d", nivo);
            ispis(4,-2.4,0.85, strcat(checkpoint, nivo_niska), 1);
        }
        else if(nivo == 20)
            //ostvaren je pogodak, a u toku je 20. nivo, to znaci da su predjeni svi nivoi
            ispis(4,-3.8,0.85, "Cestitamo, zavrsen je poslednji nivo", 1);
    }
    else
        if(status == 0){
            //usledio je promasaj, ide i odgovarajuca poruka
            ispis(3,-1.5,0.85, "Promasaj", 0);
            ispis(4,-3.5,0.85, "Pritisni 'r' za poslednji checkpoint", 1);
        }
        else
        {
            //dok traje igra pisace koji je nivo u pitanju
            char nivo_niska[10], niska[50] = "Nivo ";
            sprintf(nivo_niska, "%d", nivo == 0 ? nivo + 1 : nivo);
            ispis(3,-1.12,0.85, strcat(niska, nivo_niska), 0);
        }
    ...
    switch(key){
        ...
        case 'g':
            ...
            if ( ((animacija == 0 && odabrana_casa != 0) || start == 0) && status == 1) {
                if (nivo == 20)
                    //sprecava se pokretanje 21. nivoa
                    return;
                if(nivo == 5 || nivo == 10 || nivo == 15)
                    //azurira se checkpoint po potrebi
                    checkpoint = nivo;
                nivo ++;
                akc = -1;
                start = 1;
         ...
         //pritiskom na taster 'r' resetuju se svi podaci do checkpointa
         case 'r':
            if(nivo == 20 && pogodak == 1)
                //ako je predjena igra, brise se prethodni checkpoint i pocinje se ispocetka
                checkpoint = 0;
            nivo = checkpoint;
            animacija = 0;
            start = 0;
